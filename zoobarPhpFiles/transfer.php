<?php
  ...
  if((isset($_POST['submission'])) and ($_POST['submission'])) {
    ...
    $user->username = database_safe($user->username);
    $sql = "SELECT Zoobars FROM Person WHERE Username='$user->username'";
    $rs = $db->executeQuery($sql);
    $sender_balance = $rs->getValueByNr(0,0) - $zoobars;
    $recipient = database_safe($recipient);
    $sql = "SELECT Username, Zoobars FROM Person WHERE Username='$recipient'";
    $rs = $db->executeQuery($sql);
    $recipient_exists = $rs->getValueByNr(0,0);
    if($zoobars > 0 && $sender_balance >= 0 && $recipient_exists) {
	    $sql = "UPDATE Person SET Zoobars = $sender_balance WHERE Username='$user->username'";
      $db->executeQuery($sql);
    ...
    else $result = "Transfer to " . database_unsafe($recipient) . " failed.";
  }
?>
<p><b>Balance:</b>
