...
<?php
  $selecteduser = database_safe((isset($_GET['user'])) ? $_GET['user'] : "");
  ...
  if ( $rs->next() ) { // Sanitize and display profile
    ...
    $profile = database_unsafe($profile);
    $username = database_unsafe($username);
...
?><script type="text/javascript">
  ...
  function showZoobars(zoobars) {
    document.getElementById("profileheader").innerHTML =
      "<?php echo database_unsafe($selecteduser) ?>'s zoobars:" + zoobars;
    if (zoobars < total) {
